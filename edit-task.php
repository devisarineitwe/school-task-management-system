<script type="text/javascript">
	...
	//   document.getElementById('t_end_time').min = new Date(new Date().getTime() - new Date().getTimezoneOffset() * 60000).toISOString().split("T")[0];
		var today = new Date();
		var date = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();
		var time = today.getHours() + ":" + today.getMinutes() + ":" + today.getSeconds();
		var dateTime = date+' '+time;
		document.getElementById('t_start_time').min = dateTime
	</script>
<?php
                          ...
                          <form class="form-horizontal" role="form" action="" method="post" autocomplete="off">
                            <div class="form-group">
			                  ...
			                  <div class="form-group">
			                    ...
			                    <div class="col-sm-7">	
									<input type="datetime-local" name="t_start_time" id="t_start_time"  min="<?php echo date('Y-m-d H:i:s');?>"  class="form-control" value="<?php echo $row['t_start_time']; ?>">
			                    </div>
			                  </div>
			                  <div class="form-group">
			                    <label class="control-label col-sm-5">End Time</label>
			                    <div class="col-sm-7">
			                      <input type="datetime-local" name="t_end_time" id="t_end_time" min="<?php echo date('Y-m-d H:i:s');?>"  class="form-control" value="<?php echo $row['t_end_time']; ?>">
			                    </div>
			                  </div>
